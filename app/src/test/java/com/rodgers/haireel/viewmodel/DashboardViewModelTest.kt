...
class DashboardViewModelTest {
    ...
    fun setUp() {
        ...
        vm = DashboardViewModel(mockApp, mockDao)
    }
    ...
    fun `空のレコードリストはすべての月が0のサマリーになる`() {
        every { mockDao.recordsForYearFlow(any()) } returns flowOf(emptyList())
        val vm2 = DashboardViewModel(mockApp, mockDao)
        // stateIn の初期値を確認
    ...
    @Test
    fun `初期patternIdは空文字（全取引先）`() {
        assertEquals("", vm.patternId.value)
    }
    ...
    @Test
    fun `setPatternIdに空文字を渡すと全取引先モードに戻る`() {
        vm.setPatternId("5")
        vm.setPatternId("")
        assertEquals("", vm.patternId.value)
    }
