...
class WorkRecordTest {
    ...
    fun `beforeTimeがnullはbeforeDoneがfalse`() {
        val r = com.rodgers.routist.model.TenkoRecord(date = "2026-06-01", beforeTime = null)
        assertFalse(r.beforeDone)
    }

    @Test
    fun `beforeTimeが設定済みはbeforeDoneがtrue`() {
        val r = com.rodgers.routist.model.TenkoRecord(date = "2026-06-01", beforeTime = "08:30")
        assertTrue(r.beforeDone)
    }

    @Test
    fun `afterTimeがnullはafterDoneがfalse`() {
        val r = com.rodgers.routist.model.TenkoRecord(date = "2026-06-01", afterTime = null)
        assertFalse(r.afterDone)
    }

    @Test
    fun `afterTimeが設定済みはafterDoneがtrue`() {
        val r = com.rodgers.routist.model.TenkoRecord(date = "2026-06-01", afterTime = "18:00")
