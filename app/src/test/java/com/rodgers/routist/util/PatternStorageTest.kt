...
class PatternStorageTest {
    ...
    fun `saveで全フラグが正しく保存される`() {
        val pattern = ReportPattern(
            ...
            showRemarks = false, showTotal = false,
            paymentType = 1, unitPrice = 5000
        )
        PatternStorage.save(ctx, pattern)
        ...
        assertFalse(got.showFuel)
        assertTrue(got.showMeter)
        assertTrue(got.showIncome)
        assertFalse(got.showArea)
        assertFalse(got.showRemarks)
        assertFalse(got.showTotal)
        assertEquals(1, got.paymentType)
        assertEquals(5000, got.unitPrice)
    }

    // ── delete: 回帰テスト（col_fuel/col_meter/col_income/show_totalのキー漏れ防止）──
    ...
    fun `deleteで全キーが削除されsave後デフォルト値に戻る`() {
        val original = ReportPattern(
            id = 10, title = "削除テスト",
            showFuel = false, showMeter = true,
            showIncome = true, showTotal = false,
        ...
        assertTrue("showFuelはデフォルトtrue", restored.showFuel)
        assertFalse("showMeterはデフォルトfalse", restored.showMeter)
        assertFalse("showIncomeはデフォルトfalse", restored.showIncome)
        assertTrue("showTotalはデフォルトtrue", restored.showTotal)
