package com.rodgers.haireel.pdf
...
object PdfGenerator {
    ...
    fun generateReportPdf(
        ...
        val totalMinsP  = records.sumOf { it.workingMinutes }
        val totalHoursP = if (totalMinsP > 0) "%d時間%02d分".format(totalMinsP / 60, totalMinsP % 60) else ""
        val totalFuelP  = records.sumOf { it.fuelCost }
        val totalIncomeP = records.sumOf { it.income }
        val totalDelivP = records.sumOf { it.deliveryCount }
        val totalPkgP   = records.sumOf { it.packageCount }
        val totalDistP  = records.sumOf { it.distanceKm.toDouble() }
        ...
        val cols: List<Triple<String, (WorkRecord?) -> String, String>> = buildList {
            ...
            for (col in pattern.excelColumns) {
                when (col.type) {
                    ColumnType.START_TIME    -> add(Triple(col.label, { r: WorkRecord? -> r?.startTime ?: "" }, ""))
                    ColumnType.END_TIME      -> add(Triple(col.label, { r: WorkRecord? ->
                        if (r == null) "" else if (r.endDateOffset > 0) "${r.endTime}(+${r.endDateOffset}日)" else r.endTime
                    }, ""))
                    ColumnType.WORKING_HOURS -> add(Triple(col.label, { r: WorkRecord? -> r?.workingHoursText ?: "" }, totalHoursP))
                    ColumnType.DELIVERY_COUNT -> add(Triple(col.label,
                        { r: WorkRecord? -> if ((r?.deliveryCount ?: 0) > 0) "${r!!.deliveryCount}件" else "" },
                        if (totalDelivP > 0) "${totalDelivP}件" else ""))
                    ColumnType.PACKAGE_COUNT -> add(Triple(col.label,
                        { r: WorkRecord? -> if ((r?.packageCount ?: 0) > 0) "${r!!.packageCount}個" else "" },
                        if (totalPkgP > 0) "${totalPkgP}個" else ""))
                    ColumnType.DISTANCE      -> add(Triple(col.label,
                        { r: WorkRecord? -> if ((r?.distanceKm ?: 0f) > 0f) "%.0fkm".format(r!!.distanceKm) else "" },
                        if (totalDistP > 0) "%.0fkm".format(totalDistP) else ""))
                    ColumnType.FUEL_COST     -> add(Triple(col.label,
                        { r: WorkRecord? -> if ((r?.fuelCost ?: 0) > 0) "%,d円".format(r!!.fuelCost) else "" },
                        if (totalFuelP > 0) "%,d円".format(totalFuelP) else ""))
                    ColumnType.METER_START   -> add(Triple(col.label,
                        { r: WorkRecord? -> if ((r?.startMeter ?: 0) > 0) "${r!!.startMeter}km" else "" }, ""))
                    ColumnType.METER_END     -> add(Triple(col.label,
                        { r: WorkRecord? -> if ((r?.endMeter ?: 0) > 0) "${r!!.endMeter}km" else "" }, ""))
                    ColumnType.INCOME        -> add(Triple(col.label,
                        { r: WorkRecord? -> if ((r?.income ?: 0) > 0) "%,d円".format(r!!.income) else "" },
                        if (totalIncomeP > 0) "%,d円".format(totalIncomeP) else ""))
                    ColumnType.AREA          -> add(Triple(col.label, { r: WorkRecord? -> r?.area ?: "" }, ""))
                    ColumnType.REMARKS       -> add(Triple(col.label, { r: WorkRecord? -> r?.remarks ?: "" }, ""))
                }
            }
        }
        ...
        val totalRow = cols.mapIndexed { idx, col -> if (idx == 0) "合計（${workingDays}日稼働）" else col.third }
        ...
        drawReportTable(canvas, headers, dataRows, totalRow, widths, rowH, tableTop, daysCount, true)
