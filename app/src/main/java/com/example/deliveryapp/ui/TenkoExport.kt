...
internal fun TenkoFragment.askOrientation(ctx: android.content.Context, format: String = "ファイル", onSelected: (Boolean) -> Unit) {
    ...
    askOrientation(ctx, "Excel") { portrait ->
        lifecycleScope.launch {
            try {
                ...
                val file = TenkoExcelGenerator(ctx).generate(records, ym, portrait)
...
internal fun TenkoFragment.exportTenkoPdf() {
    ...
    askOrientation(ctx, "PDF") { portrait ->
        lifecycleScope.launch {
            try {
                val records = viewModel.recordsForMonth(ym)
                val file = PdfGenerator.generateTenkoPdf(ctx, records, ym, portrait)
                val uri = FileProvider.getUriForFile(ctx, "${ctx.packageName}.fileprovider", file)
                startActivity(Intent.createChooser(
                    Intent(Intent.ACTION_SEND).apply {
